...
const createTransaction = async (order: Order, sellOrder: Order): Promise<[Order, Order]> => {
    const requested = order.quantity; 
    order.quantity -= sellOrder.quantity;
    sellOrder.quantity -= requested;
...
const matchOrder = async (record: aws.dynamodb.TableEventRecord) => {
    if (record.dynamodb.NewImage != undefined) {
        ...
        if (orders != undefined) {
            await Promise.all(orders.map(async (sellOrder) => {
                if (order.quantity > 0) {
                    [order, sellOrder] = await createTransaction(order, sellOrder)
                    await updateOrder(sellOrder, sellsTable, "SELL")
                }
            }));
        }
        return updateOrder(order, buysTable, "BUY")
    }
}
